...
export const AppRemindModal = observer(() => {
    ...
    const hideModal = useCallback(() => {
        if (shown.current) {
            notificationStore.reduceRemindNotice();
            setVisible(false);
            setNoticeData({});
            shown.current = false;
        }
    ...
    useEffect(
        () =>
            autorun(() => {
                if (notificationStore.remindNotice.length > 0) {
                    showModal(notificationStore.remindNotice[0]);
                }
            }),
        [],
    );
